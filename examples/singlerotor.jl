...
function generate_monitor(J, rho, RPM, nsteps; save_path=nothing,
    ...
    colors="rgbcmy"^100
    stls = "o^*.px"^100
    ccb_gamma, ccb_Np, ccb_Tp = nothing, nothing, nothing
    ...
    function extra_runtime_function(sim::fvs.Simulation{V, M, R},
        ...
        if disp_conv
            ...
            clr = fcalls==0 ? (0,0,0) : (1-cratio, 0, cratio)
            stl = fcalls==0 ? "o" : "-"
            alpha = fcalls==0 ? 1 : 0.5
            ...
            # Save CCBlade solution (initial solution)
            if fcalls==0
                ccb_gamma = this_sol
            end
        ...
        if disp_conv
            # Re-plots the initial solution (CCBlade)
            if fcalls%20==0 && fcalls!=0
                for (i, data) in enumerate([ccb_gamma, ccb_Np, ccb_Tp])
                    subplot(230+i)
                    plot(1:size(data,1), data, "ok")
                end
            end
